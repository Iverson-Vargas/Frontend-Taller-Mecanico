...
export const Menu = ({ isOpen, setIsOpen }) => {
    ...
    const cargoMostrar = usuarioInfo?.cargo || (usuarioInfo?.rol === 'admin' ? 'Administrador' : 'Empleado');
    ...
    return (
        <aside className={`fixed top-0 left-0 w-72 h-screen bg-[#0f172a] text-slate-50 shadow-2xl flex flex-col z-50 overflow-hidden font-sans border-r border-slate-800/40 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>

            <div className="py-3 px-4 flex items-center gap-3 border-b border-slate-800/40">
                <div className="w-8 h-8 bg-rose-500 rounded-xl flex items-center justify-center shadow-lg shadow-rose-500/20 shrink-0">
                ...
                <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 truncate mt-0.5">{usuarioInfo?.nombre || 'Usuario'}</p>
                </div>
                <span className="ml-auto shrink-0 text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-rose-500/15 text-rose-400 border border-rose-500/20 uppercase">
                    {cargoMostrar}
                </span>
            </div>

            <nav className="flex-1 overflow-y-auto px-3" style={{ scrollbarWidth: 'thin', scrollbarColor: '#334155 transparent' }}>
                    ...
                    {menuItems.map((item, index) => (
                        <li key={index} className="w-full">
                            {item.subMenus ? (
                                <div className="mb-1">
                                    ...
                                    >
                                        ...
                                        {item.subMenus.map((subItem, subIndex) => (
                                            <li key={subIndex}>
                                                <NavLink to={subItem.path} className={estilosDelEnlace} onClick={() => setIsOpen && setIsOpen(false)}>
                                                    {subItem.label}
                                                </NavLink>
                                            </li>
                                        ))}
                                ...
                                <NavLink
                                    to={item.path}
                                    onClick={() => { setOpenMenu(null); setIsOpen && setIsOpen(false); }}
            ...
            <div className="p-3 mt-auto border-t border-slate-800/40 bg-slate-900/50 flex flex-col gap-2">
                {(permisos.admin_caja || usuarioInfo?.rol === 'admin') && (
                    <NavLink
                        to="/panel/Configuracion"
                        onClick={() => setIsOpen && setIsOpen(false)}
                        className={({ isActive }) => `flex items-center justify-center gap-2 w-full py-2 rounded-lg transition-all duration-300 font-bold text-[11px] uppercase tracking-widest border ${isActive ? 'bg-slate-700 text-white border-slate-600' : 'bg-slate-800/50 text-slate-400 border-slate-700/50 hover:bg-slate-700 hover:text-white'}`}
                    >
